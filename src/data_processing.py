...
def main(train_input, test_input, output_path, baseline_sample_path):
    ...
    This version includes all robust cleaning and feature engineering steps.
    """
    ...
    embarked_mode = combined_df.filter(col("Embarked").isNotNull()) \
                               .groupBy("Embarked") \
                               .count() \
    ...
    # --- THIS IS THE FIX ---
    # Create the baseline sample from the FINAL, PROCESSED training data.
    # This ensures the baseline has the exact same schema and data types
    # as the new data it will be compared against.
    print("\n--- Creating and saving a baseline data sample from processed data ---")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Preprocess and engineer features for Titanic Data")
    parser.add_argument("--train_input", required=True)
    parser.add_argument("--test_input", required=True)
    parser.add_argument("--output_path", required=True)
    parser.add_argument("--baseline_sample_path", required=True)
